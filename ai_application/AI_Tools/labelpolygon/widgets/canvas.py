...
from ai_application.AI_Tools.labelpolygon import QT5
from ai_application.AI_Tools.labelpolygon.shape import Shape
import ai_application.AI_Tools.labelpolygon.utils
...
class Canvas(QtWidgets.QWidget):
    ...
    def closeEnough(self, p1, p2):
        ...
        return ai_application.AI_Tools.labelpolygon.utils.distance(p1 - p2) < (self.epsilon / self.scale)
    ...
    def intersectionPoint(self, p1, p2):
        ...
        if (x, y) == (x1, y1):
            ...
            if x3 == x4:
                return QtCore.QPoint(int(x3), int(min(max(0, int(y2))), int(max(int(y3), int(y4)))))
            else:  # y3 == y4
                return QtCore.QPoint(int(min(max(0, int(x2))), int(max(int(x3), int(x4)))), int(y3))
        return QtCore.QPoint(int(x), int(y))
    ...
    def intersectingEdges(self, point1, point2, points):
        ...
        for i in range(4):
            ...
            if 0 <= ua <= 1 and 0 <= ub <= 1:
                ...
                d = ai_application.AI_Tools.labelpolygon.utils.distance(m - QtCore.QPoint(int(x2), int(y2)))
                yield d, i, (x, y)
